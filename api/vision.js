import Anthropic from "@anthropic-ai/sdk";

const client = new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY });

const PROMPT_EXTRACCION = `Analizá este/estos documento/s argentino/s.

Si recibís más de una imagen, pueden ser distintas caras del MISMO documento (ej: frente y dorso de una tarjeta verde, o dos fotos del mismo DNI) o documentos de personas distintas (ej: DNI del comprador y DNI del vendedor). Si reconocés que dos imágenes corresponden a la MISMA persona (mismo nombre y/o DNI), fusioná sus datos en una sola entrada dentro de "personas" — NUNCA la dupliques. Si son personas distintas, devolvé una entrada por cada una.

IDENTIFICAR TIPO — seguí este orden:

A) ¿Ves "DOMINIO", "CHASIS", "MOTOR", "MARCA", "MODELO" con valores técnicos? (frente de tarjeta verde/cédula de vehículo, física o digital)
   → usá FORMATO A — VEHÍCULO FRENTE

B) ¿Ves "TITULAR:" seguido de un nombre, "AUTORIZADO:" seguido de otro nombre, y el logo DNRPA? (dorso de tarjeta verde)
   → usá FORMATO B — VEHÍCULO DORSO

C) ¿Es DNI (libreta vieja, tarjeta nueva, o digital desde la app Mi Argentina), LE, LC, pasaporte, licencia, partida de nacimiento/matrimonio?
   → usá FORMATO C — PERSONA

Instrucciones: el documento puede estar en cualquier orientación — rotalo mentalmente. Puede ser físico, digital (foto de pantalla), viejo o nuevo formato. Si un dato no se lee con certeza, dejá el campo vacío.

═══ FORMATO A — VEHÍCULO FRENTE (dominio, chasis, motor) ═══
{
  "tipo_documento": "tarjeta_verde",
  "vehiculo": {
    "marca": "MARCA EN MAYÚSCULAS",
    "modelo": "modelo completo con versión (ej: GOL TREND 1.6, COROLLA XEI 2.0)",
    "tipo_desc": "tipo de carrocería — buscá el campo 'TIPO', 'CARROCERIA' o 'DESCRIPCION' en la tarjeta. Ej: SEDAN 4 PUERTAS, SEDAN 5 PUERTAS, HATCHBACK 5 PUERTAS, SUV, PICKUP, RURAL, MOTOCICLETA",
    "dominio": "patente sin espacios ni guiones (ej: ABC123 o AA123BB)",
    "chasis": "número de chasis o VIN completo",
    "motor": "número de motor completo",
    "anio": "año modelo o año de fabricación — buscá el campo 'AÑO MOD', 'AÑO FAB', 'MODEL YEAR' o año que aparezca en la descripción del modelo. Devolvé solo el número de 4 dígitos (ej: 2010)",
    "color": "color del vehículo si figura"
  },
  "titular": { "apellido": "", "nombre": "", "nro_doc": "solo números" },
  "notas": ""
}

═══ FORMATO B — VEHÍCULO DORSO (titular + autorizado) ═══
{
  "tipo_documento": "tarjeta_verde",
  "personas": [
    {
      "apellido": "apellido del TITULAR",
      "nombre": "nombre/s del TITULAR",
      "nro_doc": "DNI del titular solo números",
      "tipo_doc": "DNI",
      "genero": "M|F",
      "calle": "calle del domicilio si figura",
      "numero": "número si figura",
      "localidad": "localidad si figura",
      "departamento": "departamento si figura",
      "_rol_sugerido": "AUTORIZANTE"
    },
    {
      "apellido": "apellido del AUTORIZADO",
      "nombre": "nombre/s del AUTORIZADO",
      "nro_doc": "DNI del autorizado solo números",
      "tipo_doc": "DNI",
      "genero": "M|F",
      "_rol_sugerido": "AUTORIZADO"
    }
  ],
  "notas": ""
}

═══ FORMATO C — PERSONA (DNI, pasaporte, partidas, licencias) ═══
{
  "tipo_documento": "DNI|licencia|partida_nacimiento|partida_matrimonio|otro",
  "personas": [{
    "apellido": "",
    "nombre": "",
    "nro_doc": "solo números sin puntos",
    "tipo_doc": "DNI|LE|LC|Pasaporte",
    "genero": "M|F",
    "fecha_nac": "dd/mm/aaaa",
    "estado_civil": "",
    "nacionalidad": "argentina",
    "cuit": "",
    "calle": "nombre de la calle o avenida",
    "numero": "número de puerta/lote",
    "barrio": "nombre del barrio si figura (ej: B° Di Rocco, B° Unión y Fuerza) — NO poner en localidad",
    "manzana": "manzana si figura",
    "casa": "casa o lote si figura",
    "localidad": "nombre del pueblo, ciudad o distrito — NO confundir con barrio. En DNIs el domicilio suele tener formato 'CALLE NÚMERO - LOCALIDAD - PROVINCIA' o 'CALLE NÚMERO - LOCALIDAD - LOCALIDAD' (cuando ciudad=provincia). El primer nombre geográfico tras el número es localidad.",
    "departamento": "departamento o municipio (ej: Godoy Cruz, Maipú, Capital, Luján de Cuyo, Las Heras, Guaymallén, San Martín)",
    "provincia": "En los DNIs argentinos el domicilio suele tener el formato LOCALIDAD - DEPARTAMENTO - PROVINCIA en varias líneas. La última unidad geográfica mencionada es casi siempre la PROVINCIA. Ejemplos: 'SAN JUAN - SAN JUAN' → provincia=San Juan. 'CAPITAL - SAN JUAN' → provincia=San Juan. 'GODOY CRUZ - MENDOZA' → provincia=Mendoza. 'CAPITAL - MENDOZA' → provincia=Mendoza. 'CÓRDOBA - CÓRDOBA' → provincia=Córdoba. Si el departamento es Capital/Godoy Cruz/Maipú/Las Heras/Guaymallén/Luján de Cuyo/San Martín/San Rafael/General Alvear/Rivadavia/Tunuyán/Tupungato/Malargüe/Lavalle/Junín/Santa Rosa/La Paz → provincia=Mendoza. Mirá el texto completo del domicilio y extraé el último nombre geográfico como provincia.",
    "pais": "país si figura, omitir si es Argentina",
    "fecha_nac": "Buscá la fecha de nacimiento primero en el texto impreso. Si no está clara ahí, buscala en la banda MRZ (formato AAMMDD, ej: 710821 = 21/08/1971) pero SOLO si podés leer los 6 dígitos con total nitidez. Si no podés leer la fecha con certeza en ninguna de las dos fuentes (foto borrosa, MRZ desgastada, ángulo malo), dejá el campo vacío — NO inventes, estimes ni completes una fecha aproximada bajo ninguna circunstancia. IMPORTANTE — formato de lectura: los documentos argentinos imprimen la fecha como DD/MM/AAAA (día primero, mes segundo) — NUNCA la formato estadounidense MM/DD/AAAA. Ejemplo: si ves impreso '05/03/1990', son el 5 de marzo de 1990 — NO el 3 de mayo. No asumas formato estadounidense aunque el documento tenga números que parezcan ambiguos (ej. '04/07/1985' es el 4 de julio, no el 7 de abril). Formato de salida si la encontrás: dd/mm/aaaa"
  }],
  "notas": ""
}

Respondé SOLO con el JSON válido del formato que corresponda, sin texto adicional.`;

// imagenes: [{ data, mediaType }, ...] — 1 o más imágenes del mismo o distintos documentos
export async function extraerDocumento(imagenes) {
  if (!process.env.ANTHROPIC_API_KEY) {
    throw new Error("API key no configurada");
  }
  const response = await client.messages.create({
    model: "claude-sonnet-4-6",
    max_tokens: 1500,
    messages: [{
      role: "user",
      content: [
        ...imagenes.map(img => ({ type: "image", source: { type: "base64", media_type: img.mediaType, data: img.data } })),
        { type: "text", text: PROMPT_EXTRACCION },
      ],
    }],
  });

  const texto = response.content.find(c => c.type === "text")?.text || "{}";
  const jsonMatch = texto.match(/\{[\s\S]*\}/);
  return jsonMatch ? JSON.parse(jsonMatch[0]) : {};
}

export default async function handler(req, res) {
  if (req.method !== "POST") return res.status(405).json({ error: "Method not allowed" });

  const { imagen } = req.body;
  if (!imagen?.data || !imagen?.mediaType) return res.status(400).json({ error: "Imagen requerida" });

  try {
    const datos = await extraerDocumento([{ data: imagen.data, mediaType: imagen.mediaType }]);
    return res.status(200).json(datos);
  } catch (e) {
    console.error("Vision error:", e);
    return res.status(500).json({ error: e.message });
  }
}
