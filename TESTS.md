...
| 9 | Editor — texto de adscriptos | "Notaria/o Adscripta/o **del** Registro Notarial número..." debería decir "**al** Registro Notarial" para adscriptos (titulares sí llevan "del") — afecta 50 de 52 templates (la variable `ESCRIBANO_AL_DEL` ya existía pero sólo estaba cableada en 2); 2 de esos 50 (`cert_copia`, `fe_vida`) tenían el bug inverso, con "al" hardcodeado también para titulares | Media | 🔶 Fix listo en `scripts/fix_adscripto_al_del.sql`, pendiente de correrlo en Supabase (no tengo credenciales de escritura en esta sesión) |
